...
class APITests(unittest.TestCase):
    ...
    # HELPER METHODS
    def add_tasks(self):
        db.session.add(Task("Run around in circles", date(2015, 06, 30), 10,
                            date(2016, 2, 17), 1, 1))
        db.session.commit()
        db.session.add(Task("Win Grammy", date(2016, 1, 1), 10, date(2015, 6, 30), 1, 1))
        db.session.commit()
    ...
    def test_collection_endpoint_returns_correct_data(self):
        ...
        self.assertIn(b'Win Grammy', response.data)
